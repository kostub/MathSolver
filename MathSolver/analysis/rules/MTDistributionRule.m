...
@implementation MTDistributionRule
...
- (MTExpression*) applyToTopLevelNode:(MTExpression *)expr withChildren:(NSArray *)args {
    ...
    MTOperator* distributee = [MTDistributionRule findDistributee:oper withArgs:args leftMultipliers:leftMultipliers rightMultiplers:rightMultipliers];
    if (!distributee) {
        return expr;
    }
...
{
    if (![MTDistributionRule isDistributableOperator:op]) {
        return nil;
    }
    ...
    for (MTExpression *arg in args) {
        ...
        if (!distributee && [MTDistributionRule isDistributee:arg]) {
